...
export function useGetInfiniteMyPosts(category: PostCategory) {
  return useInfiniteQuery({
    queryFn: ({ pageParam }) => getMyPosts({ pageParam, category }),
    queryKey: [queryKeys.POST, queryKeys.GET_MY_POSTS, category],
    initialPageParam: 0,
    getNextPageParam: (lastPage, allPages) => {
      if (lastPage.paging.isLast) return undefined;
      return allPages.length;
